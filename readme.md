...
> 🧠 **使命：建立一個讓每個人都能獲得心理健康支持的數位平台**

## 📖 專案概述

MindHelp 是一個專為心理健康需求設計的全方位應用程式，結合 AI 智能對話、專業資源地圖、心理測驗評估、專家文章學習等功能，為求助者與幫助者提供完整的心理健康支持生態系統。
...
### 🤖 AI 智能對話系統
- **Session-based 會話管理** - 支援多個對話主題並行
- **上下文記憶** - AI 能記住完整對話歷史提供更好建議
- **專業建議** - 基於心理學理論的智能回應
- **治療學派推薦** - 根據使用者狀況推薦適合的治療方式

### 🗺️ 智能資源地圖
- **地理位置搜尋** - 基於使用者位置找出附近心理健康資源
- **多類型機構** - 涵蓋身心科診所、心理諮商所、免費資源等
- **專業領域篩選** - 支援 CBT、ADHD、焦慮症等專業篩選
- **評論與評分系統** - 使用者可分享真實體驗
- **收藏功能** - 儲存重要資源以便隨時查閱

### 📊 專業心理測驗
- **標準化量表** - GAD-7 焦慮量表、PHQ-9 憂鬱量表、壓力感知量表
- **智能評分系統** - 自動計算分數並提供專業解釋
- **歷史記錄追蹤** - 追蹤心理健康變化趨勢
- **個人化建議** - 根據測驗結果推薦相關文章和資源

### 📚 專家文章知識庫
- **專業內容** - 由心理師、精神科醫師撰寫的專業文章
- **智能搜尋** - 支援關鍵字、標籤、相關性排序
- **個人化收藏** - 建立個人學習資料庫
- **社交分享** - 輕鬆分享有價值的內容給他人

### 🔗 社交分享生態
- **多平台支援** - Facebook、Twitter、LINE、WhatsApp、Email
- **短連結生成** - 優化社交媒體分享體驗  
- **QR Code 分享** - 支援行動裝置快速分享
- **分享統計分析** - 追蹤內容傳播效果和使用者互動

### 🔔 智能通知系統
- **個人化設定** - 使用者可自訂通知類型和頻率
- **即時推播** - 新文章、重要更新、個人化建議
- **跨平台支援** - iOS 和 Android 推播通知
...
- **資料庫**: PostgreSQL with GORM
- **認證**: JWT Token Authentication
- **API 文件**: Swagger/OpenAPI 3.0
- **AI 整合**: OpenRouter API Integration

### 核心設計原則
- **RESTful API 設計** - 標準化的 HTTP 方法和狀態碼
- **資料隱私保護** - 嚴格的資料匿名化和加密
- **可擴展架構** - 模組化設計支援功能擴展  
- **安全優先** - 防範 SQL 注入、XSS 等常見攻擊
...
總體完成度：100%
✅ Phase 3 - 已完成功能
🔄 Session-based 聊天系統 (100%)
✅ ChatSession 資料模型 - 完整的會話管理
✅ 重構現有聊天 - 向後兼容 + 新功能
✅ 新的聊天端點：
GET /chat/sessions - 獲取會話列表
POST /chat/sessions - 創建新會話
GET /chat/sessions/{sessionId}/messages - 獲取會話訊息
POST /chat/sessions/{sessionId}/messages - 發送會話訊息
✅ 智能對話上下文 - AI 可記住對話歷史
✅ 會話統計 - 訊息數量、最後更新時間等
🔗 分享功能系統 (100%)
✅ Share & ShareClick 資料模型 - 完整的分享追蹤
✅ 分享端點：
POST /shares - 創建分享連結
GET /shares/{shareId} - 獲取分享內容
GET /users/me/shares - 使用者分享列表
GET /shares/stats - 分享統計
✅ 多平台支援 - Facebook, Twitter, LINE, WhatsApp, Email
✅ 短連結生成 - 便於社交媒體分享
✅ QR Code 生成 - 支援 QR 碼分享
✅ 點擊追蹤 - 完整的分享分析
✅ 內容支援 - 文章、地點、測驗全支援
📖 Swagger 文件更新 (100%)
✅ 完整的 API 註解 - 所有新端點都有詳細文檔
✅ 參數規格 - 完整的請求/回應定義
✅ 安全性標記 - 認證要求清楚標記
✅ 錯誤處理 - 標準化錯誤回應
🏛️ 新增資料庫架構
